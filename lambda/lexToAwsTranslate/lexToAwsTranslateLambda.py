...
def validate_phrase(phrase, source_lang, target_lang):
    if phrase is not None:
        ...
        return build_validation_result(
            False,
            "phrase",
            "Translating from {} to {}. What is the phrase you would like to translate?".format(
                source_lang.capitalize(), target_lang.capitalize()
            ),
        )
...
def translatePhrase(intent_request):
    ...
    source_lang = get_slots(intent_request)["source_lang"]["value"][
        "interpretedValue"
    ].lower()
    target_lang = get_slots(intent_request)["target_lang"]["value"][
        "interpretedValue"
    ].lower()
    ...
    if source == "DialogCodeHook":
        ...
        validation_result = validate_languages(
            source_lang, target_lang
        )  # check languages are valid and supported
        ...
        if phrase is not None:
            translation = try_ex(
                lambda: translate.translate_text(
                    Text=phrase,
                    SourceLanguageCode=sourceISO,
                    TargetLanguageCode=targetISO,
                )
            )
            ...
            logger.debug(
                "Set phrase slot to None and returning elicit slot with translation"
            )
            logger.debug(
                "Phrase={}".format(intent_request["sessionState"]["intent"]["phrase"])
            )
            ...
            return elicit_slot(
                ...
                slots,
                "phrase",
                {"contentType": "PlainText", "content": format(translatedText)},
            )
        logger.debug("Translated Phrase")
        return delegate(
            session_attributes, intent_request["sessionState"]["intent"]["slots"]
        )

    # Convert phrase to translated text
    translation = try_ex(
        lambda: translate.translate_text(
            Text=phrase, SourceLanguageCode=sourceISO, TargetLanguageCode=targetISO
        )
    )
    translatedText = str(translation.get("TranslatedText"))
